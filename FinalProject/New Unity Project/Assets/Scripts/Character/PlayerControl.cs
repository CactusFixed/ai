...
public class PlayerControl : MonoBehaviour
{
    ...
    void SwitchAnima()//切换动画
    {
        ...
        if (coll.IsTouchingLayers(Ground) || coll.IsTouchingLayers(npc))
        {
            anima.SetBool("falling", false);
            anima.SetBool("idle", true);
        }
        if(rb.velocity.y < 0.2f && !rb.IsTouchingLayers(Ground) && !rb.IsTouchingLayers(npc))
        {
            anima.SetBool("falling", true);
        }
        ...
        if (isHurt)
        {
            ...
            if (Mathf.Abs(rb.velocity.x) < 2 && coll.IsTouchingLayers(Ground))
            {
                isHurt = false;
